...
class TestMCPServerWithEngine:
    def test_list_tools(self, engine):
        ...
        tools = cast(list[dict[str, Any]], mcp.list_tools())
    ...
    def test_call_tool_detect_pii(self, engine):
        ...
        result = cast(dict[str, Any], mcp.call_tool("detect_pii", {
    ...
    def test_call_tool_validate_identifier(self, engine):
        ...
        result = cast(dict[str, Any], mcp.call_tool("validate_identifier", {
            "value": "8001011238",
    ...
    def test_call_tool_list_detectors(self, engine):
        ...
        result = cast(dict[str, Any], mcp.call_tool("list_detectors", {}))
...
class TestMCPServerEdgeCases:
    def test_handle_detect_empty_text(self):
        mcp = MCPServer.from_regions(["cz"])
    ...
    def test_handle_detect_no_pii(self):
        mcp = MCPServer.from_regions(["cz"])
    ...
    def test_handle_detect_multi_country(self):
        mcp = MCPServer.from_regions(["cz", "de"])
        ...
        types = {f["type"] for f in result["findings"]}
        assert "rodne_cislo" in types
        assert "steuer_id" in types

    def test_handle_validate_invalid_request(self):
        mcp = MCPServer.from_regions(["cz"])

        missing_value = cast(dict[str, Any], mcp.call_tool("validate_identifier", {
            "detector_name": "rodne_cislo",
        }))
        missing_detector = cast(dict[str, Any], mcp.call_tool("validate_identifier", {
            "value": "8001011238",
        }))

        assert "error" in missing_value
        assert "value" in missing_value["error"]
        assert "error" in missing_detector
        assert "detector_name" in missing_detector["error"]

    def test_server_list_detectors(self):
        mcp = MCPServer.from_regions(["cz", "de"])
        ...
        detector_names = {d["name"] for d in result["detectors"]}
        assert "rodne_cislo" in detector_names
        assert "steuer_id" in detector_names
