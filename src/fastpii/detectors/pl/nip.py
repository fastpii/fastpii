...
from fastpii.countries.pl.validators import is_valid_nip


class NIPDetector(Detector):
    registry: PatternRegistry
    CONTEXT_REGEX_PATTERN: str = r"(?i)\b(?:NIP|nip|VAT|podatkowej)\b\s*:?"
    ...
    def __init__(self, registry: PatternRegistry | None = None) -> None:
        super().__init__(
            name="nip",
            region="pl",
            description="Polish tax ID (NIP) detector with checksum validation"
        )
    ...
    def detect(self, text: str) -> list[Finding]:
        patterns = self.registry.get_patterns("nip", "pl")
        if not patterns:
            return []
    ...
    def validate(self, value: str) -> bool:
        return is_valid_nip(value)

    def _extract_metadata(self, value: str) -> dict[str, object]:
        return {"checksum_valid": is_valid_nip(value)}
