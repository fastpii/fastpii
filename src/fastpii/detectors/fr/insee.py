import re

from fastpii.core._compat import override
from fastpii.detectors.base import Detector
from fastpii.models import Finding
from fastpii.patterns import PatternRegistry, get_shared_registry
from fastpii.countries.fr.validators import is_valid_insee, extract_insee_metadata


class INSEEDetector(Detector):
    CONTEXT_PATTERN: str = r"(?i)\b(?:INSEE|NIR|s[ée]curit[ée] sociale|social|SS)\b\s*:?"
    NO_CONTEXT_CONFIDENCE: float = 0.85
    CONTEXT_WINDOW: int = 50
    registry: PatternRegistry

    def __init__(self, registry: PatternRegistry | None = None) -> None:
        super().__init__(
            name="insee",
            region="fr",
            description="French national ID (INSEE/NIR) detector with Mod-97 checksum validation"
        )
        self.registry = registry or get_shared_registry()
        self._context_regex: re.Pattern[str] = re.compile(self.CONTEXT_PATTERN)

    @override
    def detect(self, text: str) -> list[Finding]:
        patterns = self.registry.get_patterns("insee", "fr")
        if not patterns:
            return []

        pattern_def = patterns[0]
        findings: list[Finding] = []

        for match in pattern_def.compiled.finditer(text):
            value = match.group(1).upper()

            if not is_valid_insee(value):
                continue

            has_context = bool(self._context_regex.search(text[max(0, match.start() - self.CONTEXT_WINDOW):match.start()]))
            metadata = extract_insee_metadata(value)

            findings.append(Finding(
                type="insee",
                value=value,
                start=match.start(),
                end=match.end(),
                confidence=pattern_def.score if has_context else self.NO_CONTEXT_CONFIDENCE,
                region="fr",
                metadata=metadata
            ))

        return findings

    @override
    def validate(self, value: str) -> bool:
        return is_valid_insee(value)

    def _extract_metadata(self, value: str) -> dict[str, object]:
        return extract_insee_metadata(value)
